...
Every expression in Vade evaluates to a value. A value is always a string, and is interpreted as a number or a boolean depending on the operation performed on the value.
